...
public class TesteTableModel extends JFrame {
	...
	private void adicionarListeners() {
		incluir.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				} else if (jtf_cidade != null
						&& jtf_cidade.getText().equals("") == true) {
					JOptionPane.showMessageDialog(null, "Digite a cidade");
				} else if (jtf_estado != null
						&& jtf_estado.getText().equals("") == true) {
					JOptionPane.showMessageDialog(null, "Digite o estado");
				} else {
					listaPessoa.add(new Pessoa(jtf_nome.getText(), jtf_cidade.getText(),
							jtf_estado.getText()));
					limpar();
					listar();
				}
	...
	private void listar() {
		table.setModel(new MinhaTableModel(new String[] { "Nome", "Cidade",
				"Estado" }, listaPessoa.size(), 3));
				
		for (int i = 0; i < listaPessoa.size(); i++) {
			table.setValueAt(listaPessoa.get(i).getNome(), i, 0);
			table.setValueAt(listaPessoa.get(i).getCidade(), i, 1);
			table.setValueAt(listaPessoa.get(i).getEstado(), i, 2);
		}
